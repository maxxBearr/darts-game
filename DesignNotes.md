...
*Canonical design doc. Last refreshed 2026-06-08 after the Reward-cleanup + geometry-stacking pass shipped (prior refresh: Typed-shop + family-icons + map-quality-rules).*
...
**Shipped 2026-06-08 (Reward cleanup + geometry stacking — six coupled tuning changes):** `specs/2026-06-08-reward-cleanup-geometry-stacking.md`, designed in a Cowork sparring session (Max + Claude) off post-typed-shop playtest, built by Claude Code (on-disk, uncommitted at refresh; pending live confirmation). One theme — **make the item economy reward commitment instead of quietly neutralizing it, and retire the rewards that mutated knobs the map now owns.** (1) **Geometry stacks again.** The math always stacked, but the OFFER layer skipped any geometry fingerprint already owned, so you held one of each of the eight — and because the eight are pairwise zero-sum against each other (Wide Triple cancels Wide Double, the four Grow-colors renormalize within a wedge, Grow Even cancels Grow Odd), greedily taking every geometry node walked the board *back to neutral*. Removed the owned-skip (within-draw distinctness kept) so you can re-acquire an entry and commit a direction harder than one item allows; the brake is **geometric per-stack decay** (marginal effect of stack k = base × decay^(k-1), summed, so cumulative asymptotes — Color Territory's 0.30 growth at `geometry_stack_decay = 0.65` tops near +86%), implemented in the math layer (`recompute_geometry` aggregates same-fingerprint stacks → `GeometrySolver.decayed_cumulative`, largest stack undamped) with the band/angle floors still the hard backstop, plus a dead-offer gate (`geometry_stack_offer_epsilon`, stop offering once the next copy is <5% as strong as the first). Stud shows one ×N stud with the live cumulative effect. (2) **Cut Extra Dart + Extra Turn rewards** — `darts_per_turn` is baked into bailout cost / streak cadence / the 3-dart assumption and `max_turns` is set per-leg by the leg lattice (a run-wide +1 desyncs the generator's pressure math); +turn was doubly redundant since the bank already buys turns via bailout. Removed from `RewardRegistry.ALL_REWARDS`. (3) **Bailout floor → 2, as a conserving partial turn.** `bailout_min_darts = 2`: with ≥3 banked it bails full-strength as before; with 2 it buys a SHORT rescue turn worth exactly the darts paid — the leg switches onto the flat `dart_budget` seam so `budget_left` caps the turn at 2 (reads 2/3) and ends after two darts. `cost == darts added` ⇒ exact conservation, NO free dart (Max was explicit); 2 is the floor, a full turn is preferred whenever affordable. (4) **All In → Tunnel Vision relic.** The old "shop only offers modifiers" meant nothing in the typed shop; now it strikes a family from the roll for the rest of the run via a generic run-scoped `main.suppressed_families` (filtered at `_generate_shop_spots` + `MapGraph._roll_event_node`; the relic appends `&"accuracy"`). The point: accuracy is the one family with a natural *finished* state (it's redistribution, not a climb), so suppressing it concentrates every future roll into families that still advance the build and collapses events to brush+geometry (which feed each other) — identity "stop offering me aim trades, double down on board-shaping." Boss-only. (5) **Pool Widener → global +1 option.** Replaced the muddy `shop_pick_count += 1` (only some paths read it) with a run-scoped `main.option_bonus` applied to *every* choice surface — events, accuracy events, challenge rewards, shop picks; shop-eligible + stackable for a +2 ceiling. (6) **Gold relic channel in the shop.** Boss rewards became shop-purchasable so the thinned pool gains surface area and relics turn seekable — refining (not breaking) the spine: **bosses = earned run-defining prizes; shop = incremental relics you chose to fish for.** `RuleModifierReward` gained `shop_eligible` + `stackable` (orthogonal — eligibility is run-definer-vs-utility, stackable is re-buyable-vs-pulled-once-owned; `excludes[]` still handles mutual exclusion). A SEPARATE gold relic slot lights on the *full bullseye* (dead space in the shop today; bonus theming — Bigger Bull grows that very spot) at `relic_spot_chance = 0.40`, cap 1, with a dedicated bull-hit claim path (reads `bull_radii`, not the `wedge:ring` lookup) and an exhaustion fallback (bull stays dark when unrolled or every eligible relic is owned). Every shop spot still costs exactly one *throw* — no per-item price; "hit it twice" for premium feel was considered and rejected as fiddly/RNG-punishing. **Boss-only run-definers:** Glass Cannon, Tunnel Vision, Mirror Zone. **Shop-eligible utility:** Lucky Eye, Triple Outs, Bigger Bull, Pool Widener. New tunables (all exported, `##` hover): `geometry_stack_decay` 0.65, `geometry_stack_offer_epsilon` 0.05, `bailout_min_darts` 2, `relic_spot_chance` 0.40, `shop_color_relic` (gold). New `tests/test_rewards.gd`; geometry stacking cases added to `tests/test_geometry.gd`.

**Shipped 2026-06-07/08 (Typed shop + family icons + map-quality rules — Phase 03 remainder, part 1):** `specs/map/04-typed-shop-impl.md`, designed across two Cowork sparring sessions, built + verified by Claude Code (map_graph seed-sweep 18.1M checks / 0 fails). The **shop's lit spots are now family-typed** — each spot rolls one of five families (uniform), hitting it offers a choice of two items from that family; five family icons (`sprites/Icons/`: scoring/streak/accuracy/geometry/brush) render on event + challenge map nodes too (challenge shows its reward family because the deposit risk entitles an informed decision). **Rarity mirrors code reality:** scoring/streak/accuracy are rarity-laddered (rare→Triple, uncommon→Double, common→singles; icon melted into the rarity smoke), while geometry/brush are rarity-less trades on random rings with their own shaders (geometry = kaleidoscope, brush = painted). **Spine amendment:** trades became purchasable (events = free-but-rolled, shop = costs darts but you chose the family). **§4a live geometry tracking** folded in — region visuals (hotspot smoke, shop spots) re-derive from the draw geometry every reflow tick instead of caching build-time polygons, so they track wedge resizes live (the Prism-resize and geo-item-mid-shop cases). **Brush ungated + unbiased** (Max's rulings): the old affinity gate (brushes hidden until you owned a matching color streak) is gone from shop AND events — brushes roll for everyone over all four colors, distinct picks guaranteed; rationale is that the typed shop makes them seekable and brushes×geometry feed each other (paint makes boards asymmetric → geometry trades get interesting). **Four map-quality invariants** added to the generator and asserted in the seed-sweep suite: act-wide shop graph-spacing (no two shops within N edges, every placement source, mid-stretch mirror shops exempt), ≥1 lane-run challenge per act, drought breaker (no run of >3 plain legs across stretch seams or mini-branches — chosen as a hard cap over a per-leg chance because an invariant is testable), and branch divergence (a mini-branch's type composition must differ from the lane span it bypasses, else the fork is a fake choice). A density pass (events floor 1→2, lower crossover-leg weight) thinned the leg-heavy feel. Claude Code caught a latent bug from the lane→special conversions overflowing the per-path event/challenge caps and fixed it with `MapNode.is_offbudget`. *(One brush report — painted rings not resizing under Color Territory — could not be reproduced in a headless harness driving both orderings; the likely real symptom was the pre-§4a overlay desync, now fixed. Left as a watch item in CLAUDE.md: re-open only if the wedge band itself, not an overlay, is seen failing to grow in live play.)*

**Shipped 2026-06-07 (Geometry items — the GEOMETRY family):** `specs/2026-06-07-geometry-items.md`, built by Claude Code. (Was "implementation in flight" at this doc's prior refresh; now shipped — the GEOMETRY family of eight zero-sum trades, the Bigger Bull relic, the Parity Out handicap, and streaks joining the challenge reward draw. Design rationale unchanged below.)

- **A new item family: board-wide, rarity-less, zero-sum reshapes of the board's physical layout** (designed in a Cowork sparring session 2026-06-07; spec lives in CLAUDE.md until shipped → `specs/2026-06-07-geometry-items.md`). **The family proper is eight trades:** Ring Trade ×2 (a signed global triple↔double width dial — scoring-lean vs checkout-lean mirror pair; opposite stacks net out), Color Territory ×4 (Grow Red/Green/White/Black — ring bands painted the target color widen radially within their wedge, the wedge's other bands pay; zero-sum per wedge column), Parity Shift ×2 (Grow Even/Odd — angular wedge-weight multiplier, renormalized to 360°, which dissolves the adjacent-same-parity edge case; the deliberately polarizing experiment). **The family laws:** (1) *board-wide only, never single-target* — single-target geometry is a flat accuracy upgrade in disguise stacking onto the hotspot power ring, and "grow a wedge, neighbors pay" is a fake trade for the 20 (flanked by 1 and 5 — shrinking neighbors buys out the board's native punishment; Fat Wedge cut); (2) *conservation* — the board never gets bigger, so every geometry item is inherently a trade (Rim Overflow cut; parked as a future boss-reward candidate); (3) *rarity-less* — conservation eats the rarity axis: rarity elsewhere means better-priced, but a zero-sum reshape's `+` and `−` are the same area, so rarity could only be a volume knob that breaks the rarity grammar; (4) *own `Family.GEOMETRY`* — challenges stay flat-laddered, events gain geometry as a free trade family, shop sells both, the family boundary IS the tier boundary; (5) *dynamic* — rules recompute against live board state (Color Territory tracks paint incl. Prism recolor-on-hit), self-limiting via renormalization, contestable territory. **Floors protect the singles** ("the board's brakes" — narrowed singles = overscore risk near checkout, which is exactly why ring-growers are real trades): ring band + wedge angle clamp at 0.45× base, exported. **Bigger Bull = boss-reward relic** outside the family (bull's first upgrade path; eats inner single — conservation holds but it's a flat, so it's earned). **Key research finding: the long-assumed async-checkout-solver dependency was a myth** — the solver reads values only, geometry moves hit *probability* not checkout *math*; the real lift is hit detection (per-wedge ring bounds + weighted wedge boundaries, generalizing the Narrow Double pattern) + rendering + a re-flow tween, with one solver touch (the fattest-segments tiebreak reads effective areas). **New UI: studs on the board surround** — persistent reminders of active dynamic board rules (replacing the old relic-row idea; architected generically so Mirror Zone etc. can migrate later). Mini-boards stay canonical/static. **Ride-along addenda:** Wedge Swap sidelined (FlipSign precedent — its `generate()` can't honor earned rarity and it's a fake trade; challenge draw lost PLACEMENT), the Parity Out race handicap (even/odd checkout restriction on a new out-rule predicate seam; first contestable handicap), and **streaks joined the challenge draw** (new `Family.STREAK` tag on the two streak classes; both already generate at all three rarities, and the earned surface is the thematically right home for a precision-pays-precision prize — challenge families now `[SCORING, STREAK]`).

**Shipped 2026-06-04/06 (Map — Phase 01 Substrate slices 1+2+3, Phase 02 Challenge Nodes, Phase 03 Events):**

- **Slice 3 (topology v2 + incremental generation) + Phase 03 Events — built 2026-06-06** (`specs/map/01-substrate-slice3-impl.md` + `specs/map/03-events-impl.md`, designed in a Cowork session 2026-06-05/06, built by Claude Code, commit `529c1e4`; initial playtest good — full tuning pass queued as its own session). **Topology v2:** each act is now **~12 traversed nodes** — entry chokepoint → branch segments (two parallel 3–5-node runs that reconverge, equal rolled length) → pre-boss chokepoint → boss; per-traversal budget **1–3 shops / 1–3 events / 1–3 challenges**, rest legs; the two runs of a segment get contrasting type mixes (`branch_contrast`). **Agency moved to the path level** — you pick a branch's whole composition, not "event vs leg" at a one-node fork. **Challenges re-homed inline** onto branch runs (skip = the other run; hard invariant: a challenge never sits without a live parallel) — supersedes the Phase 02 §17 `fork_chance` stopgap. **Incremental per-act generation** (forced by icon-routing: a family glyph you route toward must be known when the section is shown, and brush availability is runtime/affinity-gated): `generate` seeds act 0, `generate_next_act(run_state)` appends on each boss clear with state-aware family rolls; `MapView` renders the current act only (also resolves the cramped-1501 layout). **Events:** inline, **free**, 1-of-3 picks within one icon-advertised family, **replacing the per-leg accuracy pick (removed)**. The build-steering spine's "no free typed picks" refined to **trades are free, flats are earned** — a trade's `−` governs its `+`, so free trades deepen commitment instead of climbing power. Two launch families: **accuracy** (diagonal swing table — common +6–8/−2–4, uncommon +9–11/−3–5, rare +12–14/−4–5; deliberately bigger than the old near-net-zero per-leg reshape, because scarcer picks should carry weight) and **brush** (a color-for-color trade — zero-sum board real estate; modifier-pool draw). **Rarity ramps by section, events only:** 85/10/5 → 65/20/15 → 45/30/25 (act 0/1/2). **No bank interaction at events** (shop + challenge already tax it; cost = the trade + the forgone parallel run). **Brush dropped from the challenge pool** (→ `SCORING + PLACEMENT`, the high-impact flats — the same "challenges must reward big" logic that kept accuracy out), so events (trades) and challenges (flats) never share a family ⇒ no cross-surface rarity ceiling to tune. `EventFamilyIcons` scaffolds placeholder glyphs for all four families (geometry + scoring inactive: geometry unbuilt, scoring needs a make-it-a-trade retrofit before it may be free). New files: `event_node.gd` / `event_rewards.gd` / `event_family_icons.gd` / `tests/test_events.gd` (with distribution-spread assertions per the slice-2 inert-generator lesson). **Queued next (rough order):** tuning-notes session on the full skeleton → **geometry items** (new trade family for events — since spec'd 2026-06-07, see the snapshot entry above; the assumed async-checkout-solver dependency was debunked there) → **typed shop + codex** (rest of Phase 03; shares the glyph scaffold).

- **The run is now a navigable graph** (`specs/map/01-substrate-impl.md`, built by Claude Code, commit `e71762f`). Replaces the implicit leg scheduling (`x01_game.advance_leg()` self-incrementing the target ladder; shop = `leg % shop_cadence`; boss = `leg % boss_cadence`) with an explicit, seeded, per-run **generated graph** the player routes through. New `scripts/map/`: **`MapNode`** (RefCounted graph data — type/depth/lane/act + leg-param payload + edges), **`MapGraph`** (seeded-RNG generator + traversal API, headless-testable — `tests/test_map_graph.gd`, 200 seeds × 3 levels), **`MapView`** (code-built grey-rect `Control` overlay, **type-only** — shows node *type*, never params — following the `LevelSelectScreen` instantiation pattern). Topology: a shared funnel entry → `lane_count` parallel columns (with bridge crossover *edges* + an optional in-lane reconverging **off-branch fork** — the reserved Phase 02 challenge slot, currently a duplicate leg) → a single terminal **BOSS** per act; `acts = boss_count` (501→1 / 1001→2 / 1501→3). Seam (the one invasive change): generate at `_on_run_confirmed`, show `MapView` between legs (`_show_map` / `_on_map_node_chosen`), apply the chosen node's `(target, max_turns)` via a new `x01_game.start_leg_with()`; **shop is now its own node** (leaving it returns to the map, no auto-advance — both `advance_leg()` call sites intercepted); run victory = reaching the terminal boss. Slice-1 difficulty = **ported ladder** (every node at a depth shares the tier; branches differ by type composition, not hardness). Cleanup: `frequent_shopping` reward no-op'd (`shop_cadence` now dead). Deferred-to-polish: cramped 1501 layout (needs scroll/center-on-player), orphaned shop-entry intro screen, the arrival ceremony.
- **Slice 2 — the pressure-ratio generator — shipped 2026-06-05** (`specs/map/01-substrate-slice2-impl.md`, built by Claude Code + a same-session tuning fix; headless-verified 37,783 checks / 0 fails + confirmed in playtest). Replaces the ported ladder with per-leg difficulty *texture*: `pressure = target / (turns × base_darts_per_turn × expected_per_dart(depth))`, held **flat** at `pressure_baseline` (a deliberate **control variable** so shop-throwing effectiveness + marathon/sniper felt-difficulty can later be measured without difficulty drift; a ramp is deferred to a depth-dependent `pressure_baseline`). **Seeded from the slice-1 ladder** — define it as pressure 1.0, so `expected_per_dart(depth) = baseline_target(depth)/15` and **`turns = 5` reproduces slice 1 exactly (ships at parity)**. Each leg rolls `turns ∈ [4,6]` centre-weighted; `target = (turns/5) × baseline`, snapped to X01 parity + act-window-clamped. **Per-leg, not paired lanes** (a systematically dart-starved lane would choke the bank; lane-choice meaning comes later from structure, not a designed difficulty contrast). **Leg is now defined by `max_turns`, not total `darts_fronted`** (which becomes the derived `max_turns × darts_per_turn`, live) — fixing a latent bug where the +1-dart item (`extra_dart_reward.gd`) and `two_darts_boss.gd` mutate `darts_per_turn` and slice-1's store-total-and-divide loses darts / shrinks budgets; this is also the hook the Phase 02 challenge nodes' darts-per-turn manipulation will compose with. Bosses unchanged (fixed act ceilings). **A documented blind spot is kept, not corrected:** the formula undercounts darts (more darts is more forgiving *and* gives streaks more room to compound), so equal-pressure legs aren't equally hard — the turns knob carries an intentional difficulty gradient, which is the very thing flat-pressure-as-control is meant to observe. The pressure math is exposed as reusable `MapGraph` functions (`expected_per_dart` / `pressure_of` / `baseline_target`) so Phase 02's turns↔rarity banter dial reads the same curve (no drift). **Phase 02 challenge nodes are sequenced AFTER slice 2, not merged with it** (slice 2 is the difficulty frame challenge tuning needs; building challenges on the flat ladder would feel-test against a frame about to change). Post-build catch: the default `turns_center_bias = 0.6` over the narrow [4,6] range collapsed every roll to turns=5 (lerp-then-round), so the generator shipped briefly *inert* (≡ slice 1) while bounds-only tests passed — fixed with a discrete weighting (default 0.35) + a distribution-spread test. Increasing the variance zig-zag, or adding a difficulty ramp, is pure config later (`turns_min/max`, `turns_center_bias`, `pressure_baseline`); the visible texture lives more in the **dart budget** than the target number until later acts (the 100-pt target snap eats the early-act swing). The arrival-ceremony / dart-budget-readout presentation is a deferred UX polish pass.
- **Phase 02 — Challenge Nodes — impl-spec'd + built 2026-06-05** (`specs/map/02-challenge-nodes-impl.md`, built by Claude Code, headless 43k+ checks / 0 fails; live-playtested 2026-06-06 — verdict: they need the fuller map to be judged, which slice 3 + events above now provide; the §17 frequency follow-up at the end of this bullet is superseded by slice 3's branch model, and BRUSH has since left the challenge reward pool — see the events bullet). An optional, **post-boss-1**, off-branch node where the player **wagers banked darts as the race budget** to re-clear a score they've already beaten in a tighter budget — the verb is *match concisely*, not *match big*. The 2026-06-04 "banter-table" design (pick 2/3/4 turns → rarity; deposit refunds on win) was **superseded** in the design session by a single, cleaner dial: **the deposit IS the wager IS the race budget (one pool), and a loss forfeits ALL of it** (incl. unused darts) — that forfeit-everything rule is load-bearing, because **rarity is earned by how few darts you actually finish in** (bands = thirds of the deposit range), so without it depositing max would be free insurance. Net, one number (deposit size) sets both the precision bar and the loss exposure: lean = forced-rare-or-bust with a cheap loss, fat = a graduated safety net down to a common win at a pricey loss. **Target anchor** = within −300 of the highest score the player has *cleared* (boss or leg; never above it), snapped to the leg lattice (the finer "251" lattice deferred — HUD checkout assumes hundreds). **Deposit band is derived from the slice-2 seam** (the reason `expected_per_dart` was exposed): `reliable = ceil(target / expected_per_dart(depth))`, `min ≈ reliable×0.65` (floor 5, yielding to ≤reliable for cheap targets), `max ≈ reliable+2` — target from history × E from the current (stronger) depth means an old number costs fewer darts now, keeping the band a tight ~6–13 all game; computed at *arrival* (needs the live highest-cleared), not at generation. **`darts_per_turn` rolls per node ∈ [2,5], shown before the deposit** (it's the bust-grain + streak room), and only regroups the budget into ice-tray rows (raw-dart deposit chunked into rows of dpt with a partial last row; **cap by total darts, not whole turns** — the bank stores raw darts, and bailout is the only turn-quantized path). New `scripts/map/challenge_node.gd` (`ChallengeNode` Resource, §7 export surface) + `challenge_entry_view.gd` (code-first pre-entry readout + ice-tray deposit picker). **Reward** = a typed `ScoringModifier` at the earned rarity from the modifier/shop pool (the only pool carrying `family` + `rarity` — the boss-reward pool has neither), reusing the shop-pick UI. **Engine adds** (`x01_game.gd`): a `dart_budget` total-dart-cap mode (partial final turn) + a per-race `darts_per_turn` override, with a normal leg = the `dart_budget == 0` legacy case (regression-tested at parity). **Handicaps**: `rotation` + `narrow_double` recycled as aim handicaps; **`two_darts` dropped** (it mutates `darts_per_turn`, clobbering the rolled dpt the player already saw). Bailout disabled during a challenge (a fixed wager can't be topped up); **skip is always available** (the parallel leg stays intact — tempo cost paid spatially). Deferred / queued: **live playtest** of the full entry→race→reward/forfeit loop (esp. a config-needing reward through `_finish_picker`, and lose→returns-to-map-not-game-over); the **§17 frequency follow-up** — challenges were too rare (one fork/act @ 0.6 → a 1001 run had ~40% chance of none), stopgap `fork_chance` 0.6→0.85, real fix = a per-act challenge count (cap 3) + later-column placement bias so the bank gate doesn't auto-close a fork that spawns right after the boss.
...
- **The load-bearing decision: wedge sign is intrinsic to the wedge, not to the zone.** The engine arithmetic never branches — it's always `new_remaining = remaining_score - points`, with `points` carrying its own sign. A positive wedge always subtracts, a flipped wedge always adds, in every zone. We **explicitly rejected a "mirror x01" model** (arithmetic flips sign below 0, so a vanilla triple-20 at -10 would climb to +50) — it reads cleaner but makes a wedge context-dependent, and in a stacking roguelike predictable-but-you-manage-it beats clever-but-contextual. Under the locked model that vanilla triple-20 at -10 correctly goes to **-70**. Consequence: ±1 needs **no special case in relic mode** (negative is just a normal remainder, overshoot and continue); keep the classic "can't finish on 1" bust for **vanilla play only**. **Glass Cannon conflict** (it ends the *run* on bust; this relic stops busts ending the *turn*) resolved by inventory-gated pooling exclusion (Stone-Joker / affinity-gated model) — own either, the other stops pooling in boss shops; no swap option. **Checkout helper** gates the flipped-wedge route on *reliability not existence* (min ring fatness via `_one_dart_finishable`), caps upward moves at one (never down-up-down), canonical trigger being a single-single-double over a double-triple-double. **UI:** a static `+` next to the flipped wedge number is enough for now; reuse the points-fly animation for the now-legal negative `remaining_score`. The parked map idea (category-based pool filtration) was captured separately in `specs/future/map-pool-filtration.md`.
- **Implementation + review (2026-06-01).** Shipped as `FlipSignModifier` (per-dart, held back for a dead-last `_apply_flip_pass`, follows wedge swaps), `MirrorZoneReward` (sets `x01.bust_ends_turn = false`, grants two flips), and the `bust_ends_turn`-gated bust block in `process_throw`. Review caught **one deviation**: the Glass Cannon ↔ Mirror-Zone pooling exclusion shipped *one-directional* (only the relic scanned for `glass_cannon`, so taking the relic first left Glass Cannon poolable — the exact overpowered coexistence the exclusion exists to prevent). Fixed by promoting it to a **declarative `excludes: Array[StringName]` on `RuleModifierReward`**, enforced by the base `is_applicable` and declared on both `.tres`, so it's symmetric regardless of pick order and reusable for future relic pairs. `TripleOutsReward` keeps its own one-directional "Glass Cannon supersedes" scan deliberately (redundancy, not conflict) — left as a candidate for the same treatment later.
...
## Stat Upgrade System (IMPLEMENTED — reworked to trades 2026-06-02; moved to map events 2026-06-06)

> **Update 2026-06-06 (map events):** the **per-leg pick is removed** — stat trades now live on **event nodes**
> (`specs/map/03-events-impl.md`) at a bigger *diagonal* swing (gain grows with rarity: +6–8/−2–4 common up to
> +12–14/−4–5 rare; rarity weights ramp by act section, 85/10/5 → 65/20/15 → 45/30/25). Free because they're
> trades. `_show_accuracy_pick` / `_generate_upgrades` remain in `main.gd` but nothing calls them per-leg;
> `event_rewards.gd` reuses `UPGRADE_TYPES` + `_apply_upgrade`. The "shop legs every third leg" bullet below is
> likewise obsolete (shops are map nodes since Phase 01). Historic description follows:
