...
|---|---|---|---|
| 01 | **Substrate** | Data-graph + view; first hosts the *current* run flow (legs/shops/bosses) as a navigable graph. Code-first rough visuals, art swap later. The dependency everything else needs. | **SHIPPED** — slice 1 (rough-rect topology + ported ladder, `01-substrate-impl.md`, 2026-06-04, commit `e71762f`) + slice 2 (pressure-ratio generator, `01-substrate-slice2-impl.md`, 2026-06-05). **Slice 3 SHIPPED 2026-06-06** (`01-substrate-slice3-impl.md`; Claude Code, commit `529c1e4`, playtested) — topology v2: ~12-node per-act *traversed* paths, multi-node 3–5 parallel branch runs, challenges re-homed onto branches (supersedes 02 §17), **incremental per-act generation** (state-aware family rolls; per-act render fixes the cramped-1501 layout). Deferred: arrival ceremony + art reskin (polish). |
...
| 03 | **Typed shop + codex (+ events)** | Family glyph in the smoke + hover type; the codex that teaches families; **inline event nodes** (free typed *trade* picks). The informed-shop + exposure layer. | **Events slice SHIPPED 2026-06-06** (`03-events-impl.md`; Claude Code, commit `529c1e4`, playtested): inline free events, trade-only rewards (accuracy swing + brush at launch; geometry later), section rarity ramp, per-leg free pick removed, brush dropped from the challenge pool. Typed-shop-ring rework + codex = later slices, design pending. Folds in Phase B of darts-currency. |
