...
|---|---|---|---|
| 01 | **Substrate** | Data-graph + view; first hosts the *current* run flow (legs/shops/bosses) as a navigable graph. Code-first rough visuals, art swap later. The dependency everything else needs. | **Impl spec ready** (`01-substrate-impl.md`, 2026-06-04) — slice 1 = full rough-rect topology + ported ladder; pressure-ratio generator is slice 2. Design rationale in `01-substrate.md`. |
| 02 | **Challenge nodes** | Optional consented-friction x01 races; deposit darts (ID-at-the-door), player banters turns↔rarity, earn a typed pick. Recycles benched bosses. | Design-complete this session; blocked-on-substrate for feel-testing |
