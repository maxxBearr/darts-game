...
Implementation: Claude Code on `shop` branch, with playtest debug rounds. Merged to main (latest commit: "shop and and hud fixes").
Notes: All five spec parts shipped. Shop logic lives across `main.gd` (state machine + lit-spot generation + `_in_shop` phase), `dartboard.gd` (lit-spot drawing + `check_shop_hit`), `hud.gd` (shop hover tooltip), `modifier_registry.gd` (rarity-weighted item pool draw), and `x01_game.gd` (saved-darts accumulator). The swirly shader shipped as `shaders/shop_spot.gdshader`. Two additions beyond the spec emerged during implementation: a `shop_transition_duration` board-slide between leg-end and shop, and a hover tooltip that surfaces rarity-on-hover ("Common Upgrade" / "Nothing") — a natural extension of the HUD tooltip pattern. All deferred items remain deferred.
---
