...
Implementation: Claude Code (single pass), followed by manual debugging
Notes: All three phases shipped. Notable deviations from spec language captured in this header — see "Deviations from spec" section below. Open follow-ups: ellipse references in rules slideshow (deferred), "Just Mechanics" sub-menu option (deferred, easy add if playtest shows it's wanted), Stats Reference panel and Assembly Tutorial (both still deferred from prior spec).
---

# Tutorial Revamp: Discover → Do → Understand

**Status:** Shipped 2026-05-27. Implementation TBD (Claude Code pass).
...
The Claude Code pass landed the spec faithfully but made a few design calls in flight worth noting. All accepted on debugging review:

- **Slider demos pair Range with Speed/Accuracy.** Spec called for four sliders across four moments (V Speed, H Speed, V Accuracy, H Accuracy). Implementation runs **six sliders across three paired demos**: V Range + V Speed together, H Range + H Speed together, V Accuracy + H Accuracy together. Range got folded back in. Each demo waits for both sliders' "Set" buttons before advancing.
- **"Set" keeps the value; no per-beat restore.** Spec said "snapshot-restore on Next" between slider demos. Implementation: player drags slider, clicks "Set," the new value is *kept* for the rest of the tutorial. Base stats are snapshotted at tutorial start and restored only on `stop_tutorial` (finish or skip). The result feels more rewarding — the player's adjustments matter for the throw they're about to make. Restore-at-end still protects real-run stats from bleed.
- **Meters keep bouncing during explanations.** Spec said "game pauses." Implementation uses `set_input_blocked(true)` so meters visually keep bouncing while the player reads, but clicks don't register. Better pedagogy — the player sees the moving thing while reading about it.
- **Three new `throw_mechanic` hooks added:** `set_tutorial_visual_boost(bool)`, `set_tutorial_pulse_target(string)`, `recompute_aim_dimensions()`. Spec didn't anticipate these but they were needed to make the live Range slider and the accuracy-zone-pulse work cleanly.
- **Skip Stats Walkthrough button is top-LEFT** (position 16, 16), not top-right as the spec recommended. Open UX call — may want a second look.
- **Sub-menu has a Back button** for backing out of the chooser. Sensible UX, wasn't in the spec.
- **All tutorial copy lives in an `@export var tutorial_strings: Dictionary`** — inspector-tunable. Better than scattered string literals.
...
4. **Throw 3 is densely packed by design.** It contains four slider demos (V Speed, H Speed, V Accuracy, H Accuracy) plus the accuracy-zone-breathing teach during H meter sweep plus the live ghost-dart scatter teach. This is intentional — start dense, trim from playtest. Adding beats back later is worse than removing them. (Implementation note: shipped as three paired demos, six sliders total — Range got folded in.)
