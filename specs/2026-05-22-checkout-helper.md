...
Implementation: Claude Code on `checkout-helper` branch, with a follow-up debug + design refinement pass via Cowork chat after first playtest revealed performance + logic issues. Locked/Unlocked card status shipped as a separate follow-on Claude Code pass on the same branch.
Notes: All six spec parts shipped. Two refinements during the debug pass supersede the original spec wording — (1) Setup recommendation no longer ranks by "highest new_remaining wins" — it now uses a two-tier lexicographic ranking that prefers candidates leaving the player at a 1-dart-finishable remainder next turn, tiebroken by fattest finishing target then fattest this-throw target. The original heuristic always picked S1 because it maximizes new_remaining, which is the wrong direction (higher remainder = harder next-turn checkout, not easier). The current logic uses a small `_one_dart_finishable: Dictionary` cache populated cheaply by running the 22 doubles through the preview pipeline. (2) `compute_preferred_remainders` had a multi-second pinwheel on first invocation because each of its 179 iterations created a fresh local cache; fixed by adding a shared cache across all 179 iterations and a find-first-path variant (`_solve_first`) of the recursive solver, since the precompute only needs existence rather than full path enumeration. Helper UI lives in `hud.gd` (right-side panel below stats, toggle button with three states, soft hint label). Solver lives in `scoring_modifier_manager.gd`. All deferred items remain deferred. The locked/unlocked card status that shipped alongside this spec implements the 65%-locked / 35%-toggleable roll on every modifier instance at generation time via the `UNLOCK_CHANCE` constant and `roll_toggleable()` method on `ScoringModifier`. Locked modifiers cannot be clicked off; unlocked ones can. The "Try toggling a modifier to recalculate" soft hint correctly gates on whether any toggleable modifier is active.
---
