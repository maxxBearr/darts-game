...
Status: Shipped 2026-05-23 (commit `3ecfcb9` — "reclassified items and legiibilty"). Hover stats bug followup in `48ec926`, debug flag cleanup in `bf73f33`.
Implementation: Claude Code single pass against this spec.
Notes: All six goals shipped. Deferred items (pick-card icons, streak-line animation, broken-streak feedback, DartboardGeometry refactor) remain deferred. One small placement deviation from spec — `_streak_section` was instantiated programmatically in `hud.gd` and absolute-positioned rather than added to the scene tree between `TurnScoreLabel` and `DartIndicator`. Functionally equivalent for now; if scene-tree authoring becomes important (relative positioning, scene-driven layout tweaks), revisit.
---

# Modifier Icon Language, Streak Section, & Color Streak Split
