...
Implementation: Single commit `440a04b` ("boss logic in run fix"). Manual implementation pass.
Notes: All four bugs shipped. Bug 1 landed via a much simpler fix than the spec scoped (single-line button-hide guard, no diagnostic pass needed — see "Deviations from spec" below). Bonus fixes (not in original spec) also shipped in the same commit: latent shop-pick skip-button routing, Game Over "Level Select" button, `single_bull` case added to checkout pulses, `_on_modifier_skipped` shop-continuation routing.
---

# Post-1001-Playtest Bug Fix Pass

## Deviations from spec (added at archive time)

**Bug 1 — landed as a one-line button-hide guard, not a diagnostic pass.**

The spec scoped this as investigative: add `print()` diagnostics, reproduce on a 1001 run, capture which path double-fires, then land the actual fix. In practice the implementation skipped the diagnostic pass entirely and landed:

```gdscript
func _on_next_leg() -> void:
    AuidoManager.play_ui_click()
    hud.next_leg_button.visible = false   # ← new guard
    ...
```

Root cause was the next-leg button staying clickable across the leg-transition flow, so rapid double-clicks could fire `_on_next_leg` twice — once per click. Each call ran a full `advance_leg()`, hence the leg-5 → leg-6 skip. Hiding the button at the top of the handler closes the window.

This also resolved the recession-not-applying symptom in the same commit, confirming the spec's hypothesis that the dartboard recession was a downstream casualty of the double-advance (the second `advance_leg` flow was wiping `effective_wedge_values` between boss-start and first throw). No standalone recession fix was needed.

The spec's diagnostic-prints scaffolding was never landed; instead, the existing debug prints from the boss/level/reward ship were removed from `_on_next_leg`, `_show_run_won`, and `_on_run_confirmed` in the same commit (cleanup, not part of the bug fix).

**Bonus fixes shipped in the same commit (not in spec):**

- `_on_modifier_skipped` in `main.gd` now routes to `_continue_shop_after_pick()` when `_in_shop` is true. Previously the skip button during a shop modifier pick would leak out into the no-shop branch and re-show `next_leg_button` mid-shop. Latent bug surfaced during the playtest.
- `show_shop_pick_items` in `hud.gd` now sets `_skip_modifier_button.visible = true` (was `false`). The skip button is meant to be available during shop picks.
- Game Over screen added a "Level Select" button between "Back to Assembly" and "Main Menu" (`game_over_screen.gd`, `main.gd::_on_game_over_to_level_select`). Player can now reroll a different level after losing without going all the way back through the menu.
- `_draw_checkout_pulses` got a `single_bull` case beyond the spec's `triple_wedge`/`single_wedge` additions. Same shape pattern as `double_bull` — outline at `RING_SINGLE_BULL_OUTER`. `_is_checkout_segment` got the matching `single_bull` arm. Covers Glass Cannon's "any segment closes" for the inner bull.

**Acceptance criteria checked off:**

- Bug 1: 1001 boss leg now appears at 501 (leg 5) as expected; recession boss applies its dartboard effect. 501 boss leg unaffected.
- Bug 2: Legendary panel renders gold diamonds with monogram letters; hover tooltip shows `display_name — description`. Clears on new run.
- Bug 3: Triple Outs triple-checkout segments pulse gold; Glass Cannon's single/triple/bull segments all pulse. Score readout uses checkout-gold for all valid finish rings.
- Bug 4: Rotation boss accuracy zone tracks the post-rotation wedge position. The latent `WEDGE_OFFSET_DEG` correction was applied as well (no regression observed in non-boss play).
